...
void nRF24L01P_PTX::SetTransmitPower(int Power)
{
   Device.set_tx_power(Power);
}

void nRF24L01P_PTX::SetDestinationAddress(uint64_t Address)
{
...
bool nRF24L01P_PTX::IsReadyTransmit()
{
   return (Status == STATUS_STANDBY);
}

int nRF24L01P_PTX::TransmitPacket(char *Buf, int Size)
{
...
int nRF24L01P_PTX::TransmitPacket(uint8_t *Buf, int Size)
{
   // If the device isn't powered up then there is a logic error
   if (Status == STATUS_UNDEFINED || Status == STATUS_READY_INIT || Status == STATUS_POWER_DOWN)
   {
...
int nRF24L01P_PTX::TransmitPacketNonBlocking(char *Buf, int Size)
{
   // If the device isn't powered up then there is a logic error
   if (Status == STATUS_UNDEFINED || Status == STATUS_READY_INIT || Status == STATUS_POWER_DOWN)
   {
...
bool nRF24L01P_PTX::IsTransmitFinished()
{
   return Status != STATUS_TRANSMITTING;
}

int nRF24L01P_PTX::TransmitComplete()
{
   // wait until the packet is transmitted (or some error)
...
void nRF24L01P_PTX::ReadyInitialize()
{
   Status = STATUS_READY_INIT;
}

void nRF24L01P_PTX::ReadyStandby()
{
