...
SPI com[3] = {
  {COM_MOSI, COM_MISO, COM_CLK, COM_CS1, },
  {COM_MOSI, COM_MISO, COM_CLK, COM_CS2},
  {COM_MOSI, COM_MISO, COM_CLK, COM_CS3},
};
...
bool com_check_state(int card){
    ...
    if (com_current_state[card]==RX){
        return ((c&CONFIG_PRIM_RX)!=0) && ((c&CONFIG_PWR_UP)!=0) && (ce[card]==LOW);
    }
    // TX:
    return (ce[card]==HIGH) && ((c&CONFIG_PRIM_RX)==0) && ((c&CONFIG_PWR_UP)!=0);
...
//  wait_us(100);
    for (int k=0; k<n; k++) {
      uint8_t reply = com[card].write(packet[k]);
        packet[k] = reply;
    }
//    wait_us(100);
  cs[card] = HIGH;
...
void com_set_reg5(int card, uint8_t reg, uint8_t value[5])
{
    ...
    shell_print(reg_to_str(reg));
    shell_print(" ");
    shell_print(value[0],HEX);
    shell_print("-");
    shell_print(value[1],HEX);
    shell_print("-");
    shell_print(value[2],HEX);
    shell_print("-");
    shell_print(value[3],HEX);
    shell_print("-");
    shell_println(value[4],HEX);
#endif
...
{
    for(auto& c : com) {
        c.frequency(2500000);  // com.begin(SPI_2_25MHZ, MSBFIRST, 0);
    }  
