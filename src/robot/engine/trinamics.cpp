...
// Include the TMC4671 lib with corresponding SPI transfert function (see end of
// drivers.cpp)
extern "C" {
#include <TMC4671.h>
uint8_t tmc4671_readwriteByte(uint8_t motor, uint8_t data,
...
namespace trinamics {
    ...
    void tmc4671_writeRegister16BitValue_verif(uint8_t motor, uint8_t address,
                                               uint8_t channel, uint16_t value);
    uint16_t tmc4671_readRegister16BitValue_verif(uint8_t motor,
                                                  uint8_t address,
                                                  uint8_t channel);
    int16_t tmc4671_field_read_verif(uint8_t motor, uint8_t address,
                                     uint16_t mask, int16_t shift);
    ...
    void set_speed(int id_motor, float target) {
        // Calculate the right TMC4671 speed target; according to float target
        // [m/s]
        int32_t speed =
            (int32_t)(target *
                      (500000.0f / 176.0f));  // wheel diameter is 56mm, speed
                                              // target is 500 to get one rps
    ...
    SHELL_COMMAND(set, "Set speed for one driver") {
        if (argc != 2) {
            ...
            while (!shell_available()) {
                trinamics::set_speed(atoi(argv[0]), atof(argv[1]));
                wait_us(500);
            }
        }
    }
    ...
    SHELL_COMMAND(start, "Start a driver") {
        if (argc != 1) {
            ...
            while (!shell_available()) {
                trinamics::start_motor(atoi(argv[0]));
                wait_us(500);
            }
        }
    }
    ...
    SHELL_COMMAND(motor_init, "Init a motor power board") {
        if (argc != 1) {
            ...
            while (!shell_available()) {
                trinamics::tmc4671_init(atoi(argv[0]));
                wait_us(500);
            }
        }
    }
    ...
    SHELL_COMMAND(stop, "Stop a driver") {
        if (argc != 1) {
            ...
            while (!shell_available()) {
                trinamics::stop_motor(atoi(argv[0]));
                wait_us(500);
            }
        }
    }
    ...
    void start_motor(uint8_t motor) {
        tmc4671_writeInt(motor, TMC4671_PWM_SV_CHOP,
                         0x00000007);  // Enable PWM Output
        tmc4671_writeInt(motor, TMC4671_MODE_RAMP_MODE_MOTION,
                         0x00000002);  // Velocity mode
    }

    void stop_motor(uint8_t motor, bool freewheel) {
        if (freewheel) {
            tmc4671_writeInt(motor, TMC4671_PWM_SV_CHOP,
                             0x00000000);  // 0 = PWM Disabled -> free running
        } else {
            tmc4671_writeInt(motor, TMC4671_MODE_RAMP_MODE_MOTION,
                             0x00000000);  // STOP MODE
            tmc4671_writeInt(motor, TMC4671_PID_VELOCITY_TARGET,
                             0);  // ensure no velocity
        }
    }
    ...
    // Use to averaging value of ADC to get proper offset (TODO: Improve this
    // averaging function)
    ...
    void tmc4671_init(uint8_t motor) {
         ...
         * ============ CHIP VERIFICATION ==============
         */
        tmc4671_writeInt_verif(motor, TMC4671_CHIPINFO_ADDR,
                               CHIPINFO_ADDR_SI_TYPE);
        uint32_t test = tmc4671_readInt(motor, TMC4671_CHIPINFO_DATA);

        if (test == 0x34363731) is_tmc_4671++;
        ...
        if (is_tmc_4671 != 2) {
            printf(
                "It seems that the chip is not a TMC4671-ES or a TMC4671-LA, "
                "program can't continue.\n");
            return;
        } else
            printf(
                "TMC4671 detected, configuring registers for SSL_Brushless "
                "motor control...\n");

        tmc4671_writeInt_verif(motor, TMC4671_MODE_RAMP_MODE_MOTION,
                               0x00000000);  // Ensure motion is OFF (no PWM)

        /*
         * ============ MOTOR TYPE & PWM ==============
         */

        tmc4671_writeInt_verif(
            motor, TMC4671_MOTOR_TYPE_N_POLE_PAIRS,
            0x00030008);  // 0x0003 = BLDC, 0x0008 = number of pole pairs
        tmc4671_writeInt_verif(motor, TMC4671_PWM_POLARITIES,
                               0x00000000);                       // default
        tmc4671_writeInt_verif(motor, TMC4671_PWM_MAXCNT, 3999);  // 25kHz
        tmc4671_writeInt_verif(motor, TMC4671_PWM_BBM_H_BBM_L,
                               0x00001919);  // default (25 BBM)
        // This line could be used as an enable function :
        tmc4671_writeInt_verif(
            motor, TMC4671_PWM_SV_CHOP,
            0x00000007);  // 7 = centered PWM for FOC, 0 (bit8) = Space Vector
                          // PWM disabled
        ...
        while (adc_to_be_configured) {
            ...
            tmc4671_writeInt_verif(motor, TMC4671_ADC_I_SELECT,
                                   0x24000100);  // default
            tmc4671_writeInt_verif(motor, TMC4671_dsADC_MCFG_B_MCFG_A,
                                   0x00100010);  // default
            tmc4671_writeInt_verif(motor, TMC4671_dsADC_MCLK_A, 0x20000000);
            tmc4671_writeInt_verif(motor, TMC4671_dsADC_MCLK_B, 0x00000000);
            tmc4671_writeInt_verif(motor, TMC4671_dsADC_MDEC_B_MDEC_A,
                                   0x014E014E);  // default

            ThisThread::sleep_for(100ms);  // just in case

            // Setup ADC_RAW_ADDR to show I0 and I1 ADC
            tmc4671_writeInt_verif(motor, TMC4671_ADC_RAW_ADDR,
                                   ADC_RAW_ADDR_ADC_I1_RAW_ADC_I0_RAW);
            uint32_t ADCs_raw;
            uint16_t ADC_I0, ADC_I1;

            // Do an averaging of ADC values to get proper offset (different for
            // each board)
            for (int i = 0; i < numReadings; i++) {
                ...
                while (adc_not_ok) {
                    ...
                    if ((ADC_I0 < 26000) | (ADC_I1 < 26000) | (ADC_I0 > 38000) |
                        (ADC_I1 > 38000)) {
                        adc_not_ok = true;  // Error while reading, probably
                                            // missing a byte
                    } else {
                        adc_not_ok = false;
                    }
            ...
            int32_t delta_average =
                abs(int32_t(average_I0) - int32_t(average_I1));
            if (delta_average > 3000) {
                printf(
                    "Error while computing average I0, too much delta (%ld), "
                    "trying again ...\n",
                    delta_average);
                ...
                // Write new offset.
                // Scale has been decreased (instead of default 256) to get
                // better response from PID, because hardware shunt amplifier
                // have "too much" gain.
                tmc4671_writeInt_verif(
                    motor, TMC4671_ADC_I0_SCALE_OFFSET,
                    0x003C0000 |
                        ((int32_t)average_I0));  // scale 60, offset =
                                                 // calculated average before
                tmc4671_writeInt_verif(
                    motor, TMC4671_ADC_I1_SCALE_OFFSET,
                    0x003C0000 |
                        ((int32_t)average_I1));  // scale 60, offset =
                                                 // calculated average before
            }
        }
        ...
        tmc4671_writeInt_verif(motor, TMC4671_HALL_MODE,
                               0x00000000);  // normal, all default
        ...
        tmc4671_writeInt_verif(
            motor, TMC4671_PIDOUT_UQ_UD_LIMITS,
            0x00001200);  // 20 000 (arbitrary value from live tests)
        tmc4671_writeInt_verif(
            motor, TMC4671_PID_TORQUE_FLUX_LIMITS,
            0x00001200);  // 10 000 (arbitrary value from live tests, direct
                          // impact on torque at fewer speed. Can be increase.)
        tmc4671_writeInt_verif(motor, TMC4671_PID_ACCELERATION_LIMIT,
                               0x00002710);  // 10 000, does not seems to work
        tmc4671_writeInt_verif(
            motor, TMC4671_PID_VELOCITY_LIMIT,
        ...
        tmc4671_writeInt_verif(motor, TMC4671_PID_POSITION_LIMIT_LOW,
                               0x80000001);  // no limit
        tmc4671_writeInt_verif(motor, TMC4671_PID_POSITION_LIMIT_HIGH,
                               0x7FFFFFFF);  // no limit
        ...
        tmc4671_writeInt_verif(motor, TMC4671_PID_FLUX_P_FLUX_I,
                               0x01000100);  // P=256 & I=256 (default)
        tmc4671_writeInt_verif(motor, TMC4671_PID_TORQUE_P_TORQUE_I,
                               0x01000100);  // P=256 & I=256 (default)
        ...
        tmc4671_writeInt_verif(motor, TMC4671_ABN_DECODER_MODE,
                               0x00000000);  // normal, all default
        tmc4671_writeInt_verif(motor, TMC4671_ABN_DECODER_PPR,
                               0x000007D0);  // Encoder PPR = 2000
        tmc4671_writeInt_verif(
            motor, TMC4671_VELOCITY_SELECTION,
            0x00000000);  // phi_e selected from PHI_E_SELECTION
        tmc4671_writeInt_verif(
            motor, TMC4671_POSITION_SELECTION,
            0x00000000);  // phi_e selected from PHI_E_SELECTION
        tmc4671_writeInt_verif(
            motor, TMC4671_PHI_E_SELECTION,
        ...
        last_Phi_E_Selection = (uint16_t)tmc4671_readRegister16BitValue_verif(
            motor, TMC4671_PHI_E_SELECTION, BIT_0_TO_15);

        // turn hall_mode interpolation off (read, clear bit 8, write back)
        tmc4671_writeInt_verif(
            motor, TMC4671_HALL_MODE,
            tmc4671_readInt_verif(motor, TMC4671_HALL_MODE) & 0xFFFFFEFF);

        // set ABN_DECODER_PHI_E_OFFSET to zero
        tmc4671_writeRegister16BitValue_verif(
            motor, TMC4671_ABN_DECODER_PHI_E_PHI_M_OFFSET, BIT_16_TO_31, 0);

        // read actual hall angle
        hall_phi_e_old = tmc4671_field_read_verif(
            motor, TMC4671_HALL_PHI_E_INTERPOLATED_PHI_E,
            TMC4671_HALL_PHI_E_MASK, TMC4671_HALL_PHI_E_SHIFT);

        // read actual abn_decoder angle and compute difference to actual hall
        // angle
        hall_actual_coarse_offset = tmc4671_getS16CircleDifference(
            hall_phi_e_old,
            (int16_t)tmc4671_readRegister16BitValue_verif(
                motor, TMC4671_ABN_DECODER_PHI_E_PHI_M, BIT_16_TO_31));

        // set ABN_DECODER_PHI_E_OFFSET to actual hall-abn-difference, to use
        // the actual hall angle for coarse initialization
        tmc4671_writeRegister16BitValue_verif(
            motor, TMC4671_ABN_DECODER_PHI_E_PHI_M_OFFSET, BIT_16_TO_31,
            hall_actual_coarse_offset);

        // normally MOTION_MODE_UQ_UD_EXT is only used by e.g. a wizard, not in
        // normal operation
        if (tmc4671_field_read_verif(
                motor, TMC4671_MODE_RAMP_MODE_MOTION, TMC4671_MODE_MOTION_MASK,
                TMC4671_MODE_MOTION_SHIFT) != TMC4671_MOTION_MODE_UQ_UD_EXT) {
        ...
        tmc4671_writeInt_verif(
            motor, TMC4671_PID_VELOCITY_P_VELOCITY_I,
            0x012C0050);  // P=300 & I=80 (only when using halls)
        tmc4671_writeInt_verif(motor, TMC4671_MODE_RAMP_MODE_MOTION,
                               0x00000002);  // Velocity mode
        tmc4671_writeInt_verif(motor, TMC4671_PID_VELOCITY_TARGET,
                               100);  // move the motor a bit

        // Call the periodicJob until hall angle change to set the right phi E
        // offset (see comments in TMC4671 library)
        uint16_t enc_init_loop = 0;
        while (initState != 0) {
            //        tmc4671_periodicJob(motor, 0, initMode, &initState, 0,
            //        &actualInitWaitTime, 0, &hall_phi_e_old, &hall_phi_e_new,
            //                            &hall_actual_coarse_offset,
            //                            &last_Phi_E_Selection,
            //                            &last_UQ_UD_EXT, &last_PHI_E_EXT);

            // read actual hall angle
            hall_phi_e_new = tmc4671_field_read_verif(
                motor, TMC4671_HALL_PHI_E_INTERPOLATED_PHI_E,
                TMC4671_HALL_PHI_E_MASK, TMC4671_HALL_PHI_E_SHIFT);

            // wait until hall angle changed
            if (hall_phi_e_old != hall_phi_e_new) {
                // estimated value = old value + diff between old and new
                // (handle int16_t overrun)
                int16_t hall_phi_e_estimated =
                    hall_phi_e_old + tmc4671_getS16CircleDifference(
                                         hall_phi_e_new, hall_phi_e_old) /
                                         2;

                // read actual abn_decoder angle and consider last set
                // abn_decoder_offset
                ...
                // set ABN_DECODER_PHI_E_OFFSET to actual estimated angle -
                // abn_phi_e_actual difference
                tmc4671_writeRegister16BitValue_verif(
                    motor, TMC4671_ABN_DECODER_PHI_E_PHI_M_OFFSET, BIT_16_TO_31,
            ...
            if (enc_init_loop > 20000) {
                printf(
                    "Error while trying to init ABn encoder, motor didn't move "
                    "(Halls not wired ?).\nStopping program.");
                initState = 0;
                tmc4671_writeInt_verif(
                    motor, TMC4671_PWM_SV_CHOP,
        ...
        printf(
            "ABn offset has been set to : %d\n",
            (int16_t)tmc4671_readRegister16BitValue(
                motor, TMC4671_ABN_DECODER_PHI_E_PHI_M_OFFSET, BIT_16_TO_31));
        ...
        tmc4671_writeInt_verif(motor, TMC4671_PID_VELOCITY_P_VELOCITY_I,
                               0x0BB80320);  // P=3000 & I=800 (arbitrary from
                                             // live test, good results)
        tmc4671_writeInt_verif(motor, TMC4671_PID_POSITION_P_POSITION_I,
                               0x00000000);  // P=0 & I=0

        // VALUE FOR POSITION CONTROL
        // tmc4671_writeInt_verif(motor, TMC4671_PID_VELOCITY_P_VELOCITY_I,
        // 0x01F40028); // P=500 & I=40 (arbitrary from live test, good results)
        // tmc4671_writeInt_verif(motor, TMC4671_PID_POSITION_P_POSITION_I ,
        // 0x00500000); // P=80 & I=0 (arbitrary from live test, good results)
    }
    ...
    void tmc4671_writeInt_verif(uint8_t motor, uint8_t address, int32_t value) {
        ...
        while (loop) {
            ...
            if (verif != value) {
                printf(
                    "---Error SPI while writing 0x%08lx to register 0x%04x , "
                    "trying again ...\n",
                    value, address);
    ...
    int32_t tmc4671_readInt_verif(uint8_t motor, uint8_t address) {
        ...
        while (reading_not_good) {
            ...
            if (reading_not_good) {
                printf(
                    "---Error SPI while reading register 0x%04x , trying again "
                    "...\n",
                    address);
                //            ThisThread::sleep_for(10ms);
            }
